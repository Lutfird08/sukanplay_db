...
// Middleware agar bisa menerima data JSON dan tidak kena blokir CORS
app.use(cors());
...
// 4. Ambil Roster Pemain Berdasarkan ID Tim
...
// ==========================================
// API SIMPAN MATCH LOG (DENGAN DATA BAN)
// ==========================================
app.post('/api/simpan-log', (req, res) => {
    const data = req.body;

    // UPDATE: Menambahkan team_a_bans dan team_b_bans
    ...
    const values = [
        ...
        data.teamAHeroes, 
        data.teamBHeroes, 
        data.teamABans || "", // <--- DATA BAN
        data.teamBBans || "", // <--- DATA BAN
        data.scoreA, 
        data.scoreB, 
        data.winnerId,      
        data.notes, 
...
app.get('/api/history', (req, res) => {
    const sql = `
        SELECT 
            m.id, 
            m.notes, 
            DATE_FORMAT(m.match_date, '%d %b %Y') as formatted_date, 
            m.match_time, 
            m.score_a, 
            m.score_b,
            m.team_a_heroes,  /* <-- TAMBAHAN BARU */
            m.team_b_heroes,  /* <-- TAMBAHAN BARU */
...
// ==========================================
// API DASHBOARD ANALYTICS (DENGAN DATA BAN)
// ==========================================
app.get('/api/analytics', (req, res) => {
    // UPDATE: Menambahkan kolom m.team_a_bans dan m.team_b_bans
    const sql = `
        SELECT 
            m.score_a, m.score_b, 
            m.team_a_heroes, m.team_b_heroes, 
            m.team_a_bans, m.team_b_bans,
            m.player_stats,
            tA.name AS team_home, 
            tB.name AS team_away, 
            tW.name AS winner_name
        FROM match_logs m
        LEFT JOIN teams tA ON m.team_a_id = tA.id
        LEFT JOIN teams tB ON m.team_b_id = tB.id
...
// 1. Ambil Jadwal berdasarkan ID Kompetisi
...
// 2. Tambah Jadwal Baru (Bisa dipakai untuk Bulk Input nanti)
...
// 3. Update Status Jadwal (Otomatis dipanggil saat Live Tracker / Quick Input selesai)
app.put('/api/schedules/:id/complete', (req, res) => {
    const scheduleId = req.params.id;
    const { match_log_id } = req.body; // Menerima ID dari tabel match_logs
